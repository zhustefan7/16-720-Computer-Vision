...
# def numerical_grad_checker(params,name=''):
#     e = 0.01
#     W = params['W' + name] 
...
#             curr_W = W[i][j]
#             grad_W[i][j] = ((curr_W + e) - (curr_W - e))/(2*e)
...
for k,v in params.items():
    ...
    if v.ndim == 1:
        for i in range(v.shape[0]):
                ...
                h1 = forward(x, params, 'layer1')
                probs = forward(h1, params, 'output', softmax)
                loss_forward, acc_forward = compute_loss_and_acc(y, probs)

                v[i] = vi + e
                h1 = forward(x, params, 'layer1')
                probs = forward(h1, params, 'output', softmax)
                loss_backward, acc_backward = compute_loss_and_acc(y, probs)

                v[i] = vi
                loss_grad = (loss_forward - loss_backward) / (2 * e)
